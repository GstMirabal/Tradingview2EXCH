...
| Aspect | Value |
| :--- | :--- |
| **Upstream dependencies** | `binance.spot.Spot` / `binance.error.ClientError` (external SDK), `config.settings.config` (`[binance]` section: `API_KEY`, `API_SECRET`, `LIVE_TRADING`), `django.conf.settings.BINANCE_LIVE_TRADING` |
...
| `execute_order` only places a real, capital-risking order when `settings.BINANCE_LIVE_TRADING` is `True`; otherwise it uses the SDK's `new_order_test` dry-run call. Keyed to `DEBUG` until Sprint #003 (audit T-002) | `backend/apps/Binance_Connector/services.py` |
...
| `new_order_test` vs `new_order` | Binance SDK's dry-run order-validation call vs. its live order-placement call; selected by `settings.BINANCE_LIVE_TRADING`. |
