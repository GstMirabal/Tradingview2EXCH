...
- **Nothing trades until you say so.** `[binance].LIVE_TRADING` is `false` by default, and while it is off every order goes to the SDK's dry-run endpoint (`new_order_test`). `manage.py check` reports `binance.W001` so you can see which mode you are in. Confirm your alert payloads work there before switching it on.
